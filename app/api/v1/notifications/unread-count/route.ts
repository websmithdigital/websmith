...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const rateLimitResult = await checkRateLimit(apiKeyId, ipAddress, '/api/v1/notifications/unread-count');
    ...
    const result = await client.query(
      `SELECT COUNT(*) as count FROM notifications
       WHERE LOWER(customer_email) = $1 AND is_read = false`,
      [email]
    );
    ...
    await logRequest({
      apiKeyId, endpoint: '/api/v1/notifications/unread-count', method: 'GET',
    ...
    return NextResponse.json({
      success: true,
      data: { unread_count: parseInt(result.rows[0]?.count || '0', 10) },
    }, {
      headers: {
