...
function extractHighlights(content: any): { heading: string; snippet: string }[] {
  if (Array.isArray(content)) {
    return content.map((c: any) => ({
      heading: c.heading || "Key Insight",
      snippet: Array.isArray(c.body) ? (c.body[0] || "").slice(0, 110) : "",
    }));
  }

  if (typeof content === "string") {
    const headings: { heading: string; snippet: string }[] = [];
      ...
      if (trimmed.startsWith("## ")) {
        if (currentHeading) {
          headings.push({
            ...
            snippet: currentSnippet || "Comprehensive architectural breakdown and design principles.",
          });
        }
        ...
        currentSnippet = trimmed.replace(/[*_`>]/g, "").slice(0, 110);
      }
    }

    if (currentHeading && headings.length < 3) {
      headings.push({
        heading: currentHeading,
        snippet: currentSnippet || "Core engineering strategies and architectural implementations.",
      });
    }
  ...
  return [
    { heading: "Architecture & Design", snippet: "Production-grade design patterns for high-reliability systems." },
    { heading: "Performance Optimization", snippet: "Sub-100ms response latencies and server-rendered workflows." },
    { heading: "Security & Compliance", snippet: "Automated audit trails and least-privilege governance." },
  ];
}

export default function BlogCatalogClient({ initialPosts }: BlogCatalogClientProps) {
  ...
  // Sync client-side with API in background
  useEffect(() => {
    let cancelled = false;
    fetch("/api/blogs")
      .then((res) => res.json())
  ...
  const filteredPosts = useMemo(() => {
    return posts.filter((post) => {
      ...
      const matchesSearch =
        ...
        (typeof post.tags === "string" && (post.tags as string).toLowerCase().includes(query));
  ...
  const featuredHighlights = useMemo(() => {
    ...
    return extractHighlights(featuredPost.content);
  }, [featuredPost]);
    ...
    >
          ...
          >
            {filteredPosts.map((post) => {
              const tagsArray = Array.isArray(post.tags)
                ...
                ? (post.tags as string).split(/[,\s]+/).filter(Boolean)
                : [];
                  ...
                  <div style={{ padding: "20px", display: "flex", flexDirection: "column", flex: 1, justifyContent: "space-between" }}>
                    <div>
                      ...
                      {tagsArray.length > 0 && (
                        <div style={{ display: "flex", flexWrap: "wrap", gap: "5px", marginBottom: "14px" }}>
                          {tagsArray.slice(0, 3).map((tag, tIdx) => (
                            <span
                              key={tIdx}
                              style={{
                                fontSize: "10.5px",
                    ...
                    >
                      ...
                      <div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
                        ...
                        <span style={{ fontSize: "12px", fontWeight: 600, color: isDark ? "rgba(255, 255, 255, 0.85)" : "#334155" }}>
                          {post.author?.name || "WebSmith Team"}
                        </span>
                      </div>
      ...
      <style>{`
        .wsd-blog-card:hover {
          ...
          box-shadow: 0 12px 30px -8px rgba(37, 99, 235, 0.15) !important;
        }
        .wsd-blog-card:hover .wsd-card-img {
