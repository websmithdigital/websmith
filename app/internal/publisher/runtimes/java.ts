 ...
 *
 * Orchestration-only: loads template files from template/java/,
...
const TEMPLATE_DIR = path.resolve(process.cwd(), 'app', 'internal', 'publisher', 'template', 'java');
...
export function getJavaTemplates(context: PublisherContext): Record<string, string> {
  ...
  if (!fs.existsSync(TEMPLATE_DIR)) {
    throw new Error(
      `[${context.productName}] Java template directory not found: ${TEMPLATE_DIR}. ` +
  ...
  if (!stat.isDirectory()) {
    throw new Error(
      `[${context.productName}] Java template path is not a directory: ${TEMPLATE_DIR}.`
    );
  }
  ...
  if (missingFiles.length > 0) {
    throw new Error(
      `[${context.productName}] Java template validation failed — missing mandatory files:\n` +
  ...
  if (missingSubdir.length > 0) {
    throw new Error(
      `[${context.productName}] Java template validation failed — missing mandatory assets/config:\n` +
  ...
  for (const fileName of rootFiles) {
    ...
    if (unreplaced.length > 0) {
      throw new Error(
        `[${context.productName}] Java template "${fileName}" contains unreplaced placeholders:\n` +
  ...
  if (Object.keys(templates).length === 0) {
    throw new Error(
      `[${context.productName}] Java template directory is empty: ${TEMPLATE_DIR}. ` +
