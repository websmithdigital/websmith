/**
 * PHP Runtime Generator
 *
 * Orchestration-only: loads template files from template/php/,
...
const TEMPLATE_DIR = path.resolve(process.cwd(), 'app', 'internal', 'publisher', 'template', 'php');
...
function buildPlaceholders(context: PublisherContext): PlaceholderMap {
  return {
    ...
    '${runtime}': context.runtime || 'php',
    '${generated_at}': context.generatedAt || new Date().toISOString(),
    '${product_id}': context.productId.replace(/[^a-zA-Z0-9_-]/g, ''),
    '${product_name}': context.productName,
    '${package_name}': context.productName.toLowerCase().replace(/[^a-z0-9]/g, '-'),
...
export function getPhpTemplates(context: PublisherContext): Record<string, string> {
  ...
  if (!fs.existsSync(TEMPLATE_DIR)) {
    throw new Error(
      `[${context.productName}] PHP template directory not found: ${TEMPLATE_DIR}. ` +
  ...
  if (!stat.isDirectory()) {
    throw new Error(
      `[${context.productName}] PHP template path is not a directory: ${TEMPLATE_DIR}.`
    );
  }
  ...
  if (missingFiles.length > 0) {
    throw new Error(
      `[${context.productName}] PHP template validation failed — missing mandatory files:\n` +
  ...
  if (missingSubdir.length > 0) {
    throw new Error(
      `[${context.productName}] PHP template validation failed — missing mandatory assets/config:\n` +
  ...
  for (const fileName of rootFiles) {
    ...
    if (unreplaced.length > 0) {
      throw new Error(
        `[${context.productName}] PHP template "${fileName}" contains unreplaced placeholders:\n` +
  ...
  if (Object.keys(templates).length === 0) {
    throw new Error(
      `[${context.productName}] PHP template directory is empty: ${TEMPLATE_DIR}. ` +
